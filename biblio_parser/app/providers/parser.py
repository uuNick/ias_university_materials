...
class SiteCrawler:
    ...
    #     faculties_html = self.fetch_page(faculties_url)
    #     if not faculties_html:
    #         print("Не удалось загрузить страницу факультетов")
    #         return
    #
    #     # Парсинг факультетов
    #     faculties = self.parse_communities(faculties_html)
    ...
    #         faculty_html = self.fetch_page(faculty['url'])
    #         if not faculty_html:
    #             continue
    #
    #         departments = self.parse_communities(faculty_html)
    ...
    #             materials = self.get_all_materials(dept['url'], dept['count'])
    #             print(f"Найдено материалов: {len(materials)}")
    #
    #             for mat in materials:
    #                 mat_html = self.fetch_page(mat['url'])
    #                 time.sleep(1)
    #                 if not mat_html:
    #                     continue
    #
    #                 metadata = self.parse_material_metadata(mat_html)
